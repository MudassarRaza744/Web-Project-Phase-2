<?php 
    ...
    $Query="SELECT* FROM product_data"; //selecting alla the data from the product data table
    $Query2="SELECT* FROM slider_img"; //selecting all the data from the silder table 

    $getvalues=$connection->query($Query);//another method is $get= mysqli_query($connection,$query)
    ...
    <!-- Indicators -->
      <!-- Wrapper for slides -->
        <div class="carousel-inner" role="listbox">
          <?php
          $i=0;
            while($postslider = mysqli_fetch_array($getslider)) //it get all the images from the table and store in the postslider
            {
              if($i==0)//then show nothing
                {
          ?>
